...
public class WSDoAllSender extends WSDoAllHandler {
	...
	public void invoke(final MessageContext mc) throws AxisFault {
		...
		try {
			...
			final String action = getString(WSHandlerConstants.ACTION, mc);
			if (action == null) {
				throw new AxisFault("WSDoAllSender: No action defined");
			}
			final int doAction = WSSecurityUtil.decodeAction(action, actions);
			...
			if (reqData.getUsername() == null || reqData.getUsername().equals("")) {
				final String username = (String) getProperty(reqData.getMsgContext(), WSHandlerConstants.USER);
				if (username != null) {
